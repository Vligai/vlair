...
try:
    ...
    print("Error: cryptography library not installed. Install with: pip install cryptography", file=sys.stderr)
    sys.exit(1)
...
class CertificateAnalyzer:
    ...
    def extract_certificate_info(self, cert: x509.Certificate) -> Dict:
        ...
        info = {
            ...
            "public_key": {"algorithm": cert.public_key().__class__.__name__, "size": self.get_key_size(cert)},
    ...
    def extract_extensions(self, cert: x509.Certificate) -> Dict:
        ...
        try:
            ...
            extensions["basic_constraints"] = {"ca": bc_ext.value.ca, "path_length": bc_ext.value.path_length}
        except x509.ExtensionNotFound:
...
Examples:
  ...
  python analyzer.py --file-list domains.txt --format json
    ...
    parser.add_argument("--format", "-fmt", choices=["json", "txt"], default="txt", help="Output format (default: txt)")
