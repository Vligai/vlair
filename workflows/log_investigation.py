...
class LogInvestigationWorkflow(Workflow):
    ...
    def _define_steps(self):
        self.steps = [
            WorkflowStep(name="parse_logs", description="Parse and analyze log file", tool="log_analyzer", required=True),
            WorkflowStep(
                name="detect_attacks",
            ...
            WorkflowStep(name="generate_report", description="Generate investigation report", tool="internal", required=True),
        ]
    ...
    def _detect_attacks(self, context: WorkflowContext) -> StepResult:
        ...
        if xss:
            ...
            context.scorer.add_finding(
                Severity.HIGH, f"Detected {len(xss)} XSS attempts", "log_analyzer", {"count": len(xss), "samples": xss[:3]}
            )
    ...
    def _detect_bruteforce(self, context: WorkflowContext) -> StepResult:
        ...
        return StepResult(step_name="detect_bruteforce", success=True, data={"count": len(brute_force)})
    ...
    def _check_ips(self, context: WorkflowContext) -> StepResult:
        ...
        if not ips:
            return StepResult(step_name="check_ips", success=True, data={"checked": 0, "message": "No IPs to check"})
        ...
        try:
            ...
            for ip in ips:
                ...
                if result.get("verdict") == "malicious":
                    ...
                    context.scorer.add_finding(Severity.HIGH, f"Attacker IP is known malicious: {ip}", "domain_intel", result)
