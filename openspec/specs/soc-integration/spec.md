...
> As a SOC manager, I want vlair to work with our SOAR platform, so we can incorporate it into automated playbooks.
...
```bash
# Direct SIEM search
vlair siem search "source_ip=185.234.72.14" --last 24h

# Auto-generate queries from IOCs
vlair siem query --ioc evil.com
# Output for Splunk:
#   index=* (dest="evil.com" OR query="evil.com" OR url="*evil.com*")
#   | stats count by src_ip, dest, action

# Search with investigation context
vlair siem hunt --investigation INV-2026-01-28-0042

# Export SIEM query for manual use
vlair siem query --ioc 185.234.72.14 --format splunk
vlair siem query --ioc 185.234.72.14 --format elastic
vlair siem query --ioc 185.234.72.14 --format sentinel
```
...
vlair ticket create --investigation INV-2026-01-28-0042
...
vlair ticket update INCIDENT-1234 --add-comment "Containment complete"
vlair ticket update INCIDENT-1234 --attach evidence.zip
vlair ticket update INCIDENT-1234 --status resolved

# Link investigation to existing ticket
vlair ticket link --investigation INV-2026-01-28-0042 --ticket INCIDENT-1234
```
...
```bash
vlair notify slack --channel #soc-alerts --investigation INV-2026-01-28-0042
...
vlair soar trigger --playbook phishing-response \
  --input investigation=INV-2026-01-28-0042

# Check playbook status
vlair soar status --playbook-run PR-12345

# Export investigation for SOAR import
vlair investigate results INV-2026-01-28-0042 --format soar-xsoar
vlair investigate results INV-2026-01-28-0042 --format soar-phantom

# Receive SOAR webhook (for bidirectional integration)
vlair soar webhook-server --port 8080
```
...
vlair block evil.com

# Block with specific targets
vlair block 185.234.72.14 --targets firewall,proxy,edr

# Block from investigation
vlair block --investigation INV-2026-01-28-0042 --all-iocs

# Preview what would be blocked
vlair block evil.com --dry-run

# Unblock (with audit trail)
vlair unblock evil.com --reason "False positive confirmed" --ticket INC-1234

# Check block status
vlair block status evil.com
```
...
vlair tip push --platform misp \
  --event "Cobalt Strike Campaign Jan 2026" \
  --iocs-file iocs.json

# Pull latest threat intel
vlair tip pull --platform misp --tags "apt,ransomware" --last 24h

# Check IOC against TIP
vlair tip lookup 185.234.72.14

# Sync investigation IOCs to TIP
vlair tip sync --investigation INV-2026-01-28-0042
```
...
vlair integrations test splunk_prod
vlair integrations test --all
```
...
├─────────────────────────────────────────────────────────────────┤
│  CLI Commands                                                   │
│  vlair siem | ticket | notify | block | soar | tip            │
├─────────────────────────────────────────────────────────────────┤
│  Integration Manager                                            │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐            │
...
```bash
# SIEM
vlair siem search "<query>" [--source splunk|elastic] [--last 24h]
vlair siem query --ioc <ioc> [--format splunk|elastic|sentinel]
vlair siem hunt --investigation <id>

# Ticketing
vlair ticket create --investigation <id>
vlair ticket create --title "..." --priority high --assignee @team
vlair ticket update <ticket-id> --add-comment "..." [--attach file]
vlair ticket link --investigation <id> --ticket <ticket-id>

# Communication
vlair notify slack --channel #channel --message "..."
vlair notify slack --channel #channel --investigation <id>
vlair notify teams --channel "..." --message "..."
vlair notify email --to team@company.com --subject "..." --body-file report.md

# Blocking
vlair block <ioc> [--targets firewall,proxy,edr]
vlair block --investigation <id> --all-iocs
vlair unblock <ioc> --reason "..." --ticket <ticket-id>
vlair block list [--last 7d]
vlair block status <ioc>
vlair block history <ioc>

# SOAR
vlair soar trigger --playbook <name> --input key=value
vlair soar status --playbook-run <id>
vlair soar list-playbooks

# Threat Intel
vlair tip push --platform misp --event "..." --iocs-file iocs.json
vlair tip pull --platform misp --tags "apt" --last 24h
vlair tip lookup <ioc>

# Management
vlair integrations status
vlair integrations test <name>
vlair integrations config
```
