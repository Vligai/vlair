...
class AIResponseCache:
    ...
    def get(self, cache_key: str) -> Optional[AIResponse]:
        ...
        with sqlite3.connect(self.cache_path) as conn:
            ...
            if time.time() - created_at > self.ttl:
                ...
                conn.execute(
                    "DELETE FROM ai_responses WHERE cache_key = ?",
                    (cache_key,)
                )
                conn.commit()
                return None
    ...
    def clear_expired(self) -> int:
        ...
        with sqlite3.connect(self.cache_path) as conn:
            cursor = conn.execute(
                "DELETE FROM ai_responses WHERE created_at < ?",
                (cutoff,)
            )
            conn.commit()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.cache_path) as conn:
            ...
            cutoff = int(time.time()) - self.ttl
            valid = conn.execute(
                "SELECT COUNT(*) FROM ai_responses WHERE created_at >= ?",
                (cutoff,)
            ).fetchone()[0]

            # Total tokens cached
            tokens = conn.execute(
                "SELECT SUM(tokens_used) FROM ai_responses"
            ).fetchone()[0] or 0

            # Models used
            models = conn.execute(
                "SELECT DISTINCT model FROM ai_responses"
            ).fetchall()
