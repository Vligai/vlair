...
class InputDetector:
    ...
    def _detect_file_type(self, file_path: str) -> Dict[str, Any]:
        ...
        if ext in self.EMAIL_EXTENSIONS:
            return self._result(InputType.EMAIL, file_path, 'high', {
                'filename': path.name,
                'extension': ext
            })

        if ext in self.PCAP_EXTENSIONS:
            return self._result(InputType.PCAP, file_path, 'high', {
                'filename': path.name,
                'extension': ext
            })

        if ext in self.LOG_EXTENSIONS:
            return self._result(InputType.LOG, file_path, 'high', {
                'filename': path.name,
                'extension': ext
            })

        if ext in self.SCRIPT_EXTENSIONS:
            return self._result(InputType.SCRIPT, file_path, 'high', {
                'filename': path.name,
                'extension': ext,
        ...
        if ext in {'.txt', '.csv', '.ioc', '.iocs'}:
            if self._looks_like_ioc_list(file_path):
                return self._result(InputType.IOC_LIST, file_path, 'medium', {
                    'filename': path.name
                })

        # Default to generic file
        return self._result(InputType.FILE, file_path, 'low', {
            'filename': path.name,
            'extension': ext,
            ...
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                for pattern in self.LOG_PATTERNS:
                    if pattern.search(content):
                        return self._result(InputType.LOG, file_path, 'medium', {
                ...
                if 'From:' in content and ('To:' in content or 'Subject:' in content):
                    return self._result(InputType.EMAIL, file_path, 'medium', {
                        'filename': Path(file_path).name,
                        'detected_by': 'content'
                    })
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    if (self.MD5_PATTERN.match(line) or
                        self.SHA1_PATTERN.match(line) or
                        self.SHA256_PATTERN.match(line) or
                        self.IPV4_PATTERN.match(line) or
                        self.DOMAIN_PATTERN.match(line) or
                        self.URL_PATTERN.match(line)):
                        ioc_count += 1
    ...
    def _result(self, input_type: str, value: str, confidence: str,
