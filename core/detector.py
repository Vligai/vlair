    ...
    def _detect_file_type(self, file_path: str) -> Dict[str, Any]:
        ...
        if ext in self.SCRIPT_EXTENSIONS:
            return self._result(
                ...
                file_path,
                "high",
                {"filename": path.name, "extension": ext, "language": self._detect_script_language(ext)},
            )
        ...
        return self._result(
            InputType.FILE, file_path, "low", {"filename": path.name, "extension": ext, "size": path.stat().st_size}
        )
            ...
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                ...
                for pattern in self.LOG_PATTERNS:
                    if pattern.search(content):
                        return self._result(
                            InputType.LOG, file_path, "medium", {"filename": Path(file_path).name, "detected_by": "content"}
                        )

                # Check for email headers
                if "From:" in content and ("To:" in content or "Subject:" in content):
                    return self._result(
                        InputType.EMAIL, file_path, "medium", {"filename": Path(file_path).name, "detected_by": "content"}
                    )
