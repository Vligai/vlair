...
class Finding:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"severity": self.severity.value, "message": self.message, "source": self.source, "details": self.details or {}}
...
class RiskScorer:
    ...
    SEVERITY_WEIGHTS = {Severity.CRITICAL: 30, Severity.HIGH: 15, Severity.MEDIUM: 5, Severity.LOW: 2, Severity.INFO: 0}

    # Maximum contribution per severity (prevents single category from maxing score)
    SEVERITY_CAPS = {Severity.CRITICAL: 75, Severity.HIGH: 40, Severity.MEDIUM: 15, Severity.LOW: 5, Severity.INFO: 0}

    def __init__(self):
        self.findings: List[Finding] = []

    def add_finding(self, severity: Severity, message: str, source: str, details: Optional[Dict[str, Any]] = None):
    ...
    def add_findings_from_domain_intel(self, result: Dict[str, Any]):
        ...
        if verdict == "MALICIOUS" or risk_score >= 70:
            ...
            self.add_finding(
                Severity.HIGH, f"Domain/IP is suspicious (risk: {risk_score})", "domain_intel", {"risk_score": risk_score}
            )
    ...
    def add_findings_from_url_analysis(self, result: Dict[str, Any]):
        ...
        if verdict == "MALICIOUS" or risk_score >= 70:
            self.add_finding(
                Severity.CRITICAL, f"URL flagged as malicious (risk: {risk_score})", "url_analyzer", {"risk_score": risk_score}
            )
        elif verdict == "SUSPICIOUS" or risk_score >= 40:
            self.add_finding(
                Severity.HIGH, f"URL is suspicious (risk: {risk_score})", "url_analyzer", {"risk_score": risk_score}
            )
        ...
        if patterns:
            self.add_finding(
                Severity.MEDIUM, f"URL has {len(patterns)} suspicious patterns", "url_analyzer", {"patterns": patterns}
            )
    ...
    def add_findings_from_email_analysis(self, result: Dict[str, Any]):
        ...
        if spf.get("result") == "fail":
            self.add_finding(Severity.HIGH, "SPF validation failed - sender may be spoofed", "eml_parser", {"spf_result": spf})
    ...
    def add_findings_from_yara_scan(self, result: Dict[str, Any]):
        ...
        for match in matches:
            ...
            self.add_finding(sev, f"YARA rule matched: {rule}", "yara_scanner", {"rule": rule, "meta": match.get("meta", {})})
    ...
    def add_findings_from_log_analysis(self, result: Dict[str, Any]):
        ...
        if sqli:
            self.add_finding(
                Severity.CRITICAL, f"Detected {len(sqli)} SQL injection attempts", "log_analyzer", {"count": len(sqli)}
            )

        # XSS attempts
        xss = threats.get("xss", [])
        if xss:
            self.add_finding(Severity.HIGH, f"Detected {len(xss)} XSS attempts", "log_analyzer", {"count": len(xss)})

        # Path traversal
        traversal = threats.get("path_traversal", [])
        if traversal:
            self.add_finding(
                Severity.HIGH, f"Detected {len(traversal)} path traversal attempts", "log_analyzer", {"count": len(traversal)}
            )
        ...
        if brute:
            self.add_finding(
                Severity.MEDIUM, f"Detected brute force patterns from {len(brute)} IPs", "log_analyzer", {"count": len(brute)}
            )
    ...
    def add_findings_from_pcap_analysis(self, result: Dict[str, Any]):
        ...
        if c2:
            self.add_finding(
                Severity.CRITICAL, f"Detected potential C2 communication patterns", "pcap_analyzer", {"indicators": c2}
            )
    ...
    def add_findings_from_deobfuscation(self, result: Dict[str, Any]):
        ...
        if iocs:
            ...
            if total_iocs > 0:
                self.add_finding(
                    Severity.MEDIUM, f"Found {total_iocs} IOCs in deobfuscated script", "deobfuscator", {"iocs": iocs}
                )
...
def main():
    ...
    scorer.add_finding(
        Severity.CRITICAL, "Hash detected as malicious by 45/70 AV engines", "hash_lookup", {"detections": 45, "total": 70}
    )
