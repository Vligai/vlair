...
Examples:
  ...
  python extractor.py report.txt --defang --output safe_report.txt
    ...
    parser.add_argument(
        "--types", "-t", help="IOC types to extract (comma-separated): ip,domain,url,email,hash,cve,all (default: all)"
    )

    parser.add_argument(
        "--format", "-f", choices=["json", "csv", "txt", "stix"], default="json", help="Output format (default: json)"
    )
...
def main():
    ...
    extractor = IOCExtractor(
        exclude_private_ips=args.no_private_ips, whitelist_file=args.whitelist, refang=args.refang, defang=args.defang
    )
